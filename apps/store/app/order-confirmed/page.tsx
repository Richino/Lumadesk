...
export default async function OrderConfirmedPage({ searchParams }: OrderConfirmedPageProps) {
  ...
  if (sessionId?.startsWith("cs_")) {
    try {
      ...
      if (session.status === "complete" && session.payment_status === "paid") {
        ...
        const { data: order } = await admin.from("orders").select("id").eq("stripe_checkout_session_id", session.id).maybeSingle();
        state = order ? "confirmed" : "payment_received";
  ...
  return <main className="auth-page"><section className="auth-card"><p className="eyebrow">{copy.eyebrow}</p><h1>{copy.title}</h1><p className="auth-intro">{copy.body}</p><Link className="auth-submit" href="/">Return to LumaDesk</Link></section></main>;
}
